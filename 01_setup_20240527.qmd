...
# ACP regression output 
claims <- readr::read_csv("data/01_raw/claims_line_file.csv") %>%
  as_tibble()

people <- readr::read_csv("data/01_raw/patient_level_file.csv") %>%
  as_tibble()
...
perform_regression <- function(outcome_vars, covariates, data) {
  results <- list()
  ...
  for (outcome_var in outcome_vars) {
    for (i in seq_along(covariates)) {
      ...
      formula <- as.formula(paste(outcome_var, "~", paste(covariate_set, collapse = "+")))
      model <- lm(formula, data = data)
      
      result_name <- paste(outcome_var, "_vs_", paste(covariate_set, collapse = "_and_"), sep = "")
      
      
      result <- broom::tidy(model)
      result$outcome_name <- outcome_var
  ...
  for (outcome_var in outcome_vars) {
    final_results$model <- gsub(paste0("^", outcome_var, "_vs_"), "y_vs_", final_results$model)
  }
...
covariates <- list("dual_eligible", "number_comorbidities", age_sex)

# Perform regression for the variables and flatten results 
regression_results <- perform_regression(outcome_vars, covariates, a5)
...
plot <- ggplot(reg_res, aes(x = factor(model, labels = c("m1", "m2", "m3")), y = pct_change)) +
  geom_point(aes(colour = factor(outcome_name, labels = c(outcome_vars))), size = 3, position=position_dodge(width = 1)) +
 ...
 geom_text(aes(label = format(round(pct_change * 100, 1), nsmall = 1)), position=position_dodge(width = 1), vjust = -1, size = 4) +
  facet_wrap(~factor(outcome_name, labels = c('cgm any', 'rpm any')), nrow = 1) +
